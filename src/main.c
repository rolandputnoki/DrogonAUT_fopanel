...
/****************************************************/
		...
		case UTCA_SAROK_JOBB_TOLATAS_VEGE:
			...
			if(utca_sarok_most_indulunk_elore){
				...
				if(utca_sarok_utani_eloremenet_hossza >= 800){
					state_of_robot = JUST_GOING;
				}
			}
...
/****************************************************/
		...
		case KONVOJ_JELZES_JOBB_KOZELEDES:
			...
			if(konvoj_kozeledes_eloszor){
				set_compare_value_digit_szervo(KONVOJ_JOBB_KOZELEDES_SZERVO_SZOG);
				sebesseg_szabalyzas_elore_on = 0;
				set_gyari_motor_compare_value(6510);
...
//			set_compare_value_digit_szervo(32500);
		...
		case KONVOJ_JELZES_JOBB_KORRE_MENET:
...

			{
				...
				set_compare_value_digit_szervo(KONVOJ_JOBB_KORRE_MENET_SZERVO_SZOG);
	...
	//				wanted_speed = 0.5f;
		...
		case KONVOJ_JELZES_JOBB_KOVETES:
			...
			if(!megtettuk_a_kort)
			{
				...
				if(konv_elso_iven){
					set_compare_value_digit_szervo(KONVOJ_JOBB_KOVETES_ELSO_IV);
				}


				if(konvoj_von_hossz >= 9100 && !konv_masodik_iven){
					konv_elso_iven = 0;
					set_compare_value_digit_szervo(KONVOJ_JOBB_KOVETES_MASODIK_IV);
				}
		...
		case KONVOJ_KOVETKEZIK_JELZES_A_BAL_OLDALON:
			...
			set_gyari_motor_compare_value(5600);
			if(speed_of_drogon <= 0.01f){
				state_of_robot = KONVOJ_JELZES_BAL_KOZELEDES;
			}
		...
		case KONVOJ_JELZES_BAL_KOZELEDES:
			if(teszt_helyzet)
			{
				BT_UART_SendString("KONV B KÖZ\r\n");
			}

			kormany_szabalyzas_on = 0;
			if(konvoj_kozeledes_eloszor){
				set_compare_value_digit_szervo(KONVOJ_BAL_KOZELEDES_SZERVO_SZOG);
				sebesseg_szabalyzas_elore_on = 0;
				set_gyari_motor_compare_value(6510);
				konvoj_kozeledes_eloszor = 0;
				konvoj_koz_kezdet_encoder_ertek = get_encoder_counter();
				...
				if(konvoj_koz_hossz >= 400){
					set_gyari_motor_compare_value(6200);
					state_of_robot = KONVOJ_JELZES_BAL_RAALLAS;
				}
			}
...
/*
		...
		case KONVOJ_JELZES_BAL_KORRE_MENET:

			if(teszt_helyzet)
			{
				BT_UART_SendString("KONV B KORRE\r\n");
			}
...

			{
				set_gyari_motor_compare_value(6520);
				konvoj_vonalra_allas_eloszor = 0;
				set_compare_value_digit_szervo(KONVOJ_BAL_KORRE_MENET_SZERVO_SZOG);
				konvoj_von_kezdet_encoder_ertek = get_encoder_counter();
			} else {
				konvoj_von_mostani_encoder_ertek = get_encoder_counter();
				konvoj_von_hossz = (konvoj_von_kezdet_encoder_ertek - konvoj_von_mostani_encoder_ertek)*ENCODER_VALUE_TO_MM;
				if(konvoj_von_hossz >= 400){
					kormany_szabalyzas_on = 1;
//					BT_UART_SendString("KORM ON\r\n");
					state_of_robot = KONVOJ_JELZES_BAL_KOVETES;
				}
		...
		case KONVOJ_JELZES_BAL_KOVETES:
			...
			if(!megtettuk_a_kort)
			{
				...
				if(konv_elso_iven){
					set_compare_value_digit_szervo(KONVOJ_BAL_KOVETES_ELSO_IV);
				}


				if(konvoj_von_hossz >= 9500 && !konv_masodik_iven){
					konv_elso_iven = 0;
					set_compare_value_digit_szervo(KONVOJ_BAL_KOVETES_MASODIK_IV);
				}

				if(konvoj_von_hossz >= 9700)
				{
					konv_masodik_iven = 1;
					kormany_szabalyzas_on = 1;
					state_of_robot = JUST_GOING;
...
void konvoj_elhaladas_felismeres_korbe()
{
	...
	if(state_of_robot == KONVOJ_JELZES_JOBB_RAALLAS){
		...
		if(varakozasi_ido >= 800){
			state_of_robot = KONVOJ_JELZES_BAL_KORRE_MENET;
			varakozasi_ido = 0;
			autot_erzekeltem = 0;
			auto_van = 0;
			szunet_van = 0;
...
//Cél dupla keresztvonalának felismerése
//Majd egy kicsi visszatolatás
uint8_t elobb_nem_volt_vonal = 0;
uint8_t hanyszor_nem_volt_vonal = 0;
uint8_t mar_volt_vasuti_atjaro = 0;


uint8_t vasut_masodik_megallas_utani_kereszt_meg_volt = 0;
			...
			if(egyes_vonal_volt_a_kereszt_utan)
			{
				if(kereszt_vonal_utani_egyes_hossza <= 50)
				{
					...
					*/
					...
					if(state_of_robot == JUST_GOING)
					{
						if(!mar_volt_vasuti_atjaro)
						{
							state_of_robot = VASUTI_ATJARO_KOVETKEZIK;
							mar_volt_vasuti_atjaro = 1;
						}
						hanyszor_volt_kereszt_vonal = 0;
						kereszt_vonal_van = 0;
						elobb_kereszt_vonal_volt = 0;
					...
					{
//						BT_UART_SendString("DUP VON nem\r\n");
						state_of_robot = JUST_GOING;
						hanyszor_volt_kereszt_vonal = 0;
						kereszt_vonal_van = 0;
						elobb_kereszt_vonal_volt = 0;
			...
			if(hanyszor_volt_kereszt_vonal >= 1){
				...
				if(state_of_robot == VASUTI_ATJARO_KETSZER_ATHALADTUNK && vasut_masodik_megallas_utani_kereszt_meg_volt == 0)
				{

						vasut_masodik_megallas_utani_kereszt_meg_volt = 1;
				} else {
					kereszt_vonal_van = 1;
					hanyszor_volt_kereszt_vonal = 0;
					elobb_kereszt_vonal_volt = 0;
			...
			if(vonalak_szama() == 1){
				...
				else
				{
					...
					if(kereszt_vonal_utani_egyes_hossza >= 50)
					{
						...
						kereszt_vonal_utani_egyes_hossza = 0;
						egyes_vonal_volt_a_kereszt_utan = 0;
//						BT_UART_SendString("KERESZT U EGY\r\n");
			...
			if(vil_ledek_szama() == 0)
			{
//				BT_UART_SendString("KERESZT U NULLA\r\n");
				if(!elobb_nem_volt_vonal)
				{
					elobb_nem_volt_vonal = 1;
				} else {
					hanyszor_nem_volt_vonal++;
				}

				if(hanyszor_nem_volt_vonal >= 15)
				{
					if(state_of_robot == JUST_GOING)
					{
...
*/
		...
		if(jobb_oldali_sharp_szenzor >= 400 && bal_oldali_sharp_szenzor <= 300){
